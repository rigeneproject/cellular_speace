...
def test_gene_expression_profile_model():
    p = GeneExpressionProfile(cell_id="c1", stress_response_genes=["hsp70_like"])
    assert "hsp70_like" in p.stress_response_genes
...
def test_stressed_cell_gets_stress_genes():
    circuit = _make_circuit()
    stress_engine = CellularStressEngine()
    stress_result = stress_engine.evaluate(circuit)
    damage_engine = CellularDamageEngine()
    ...
    assert len(n2_profile.stress_response_genes) > 0


def test_healthy_cell_has_metabolic_genes():
    circuit = _make_circuit()
    stress_engine = CellularStressEngine()
    stress_result = stress_engine.evaluate(circuit)
    damage_engine = CellularDamageEngine()
    ...
    assert len(n1_profile.metabolic_genes) > 0
...
def test_epigenetic_marks_persisted_on_neuron():
    circuit = _make_circuit()
    stress_engine = CellularStressEngine()
    stress_result = stress_engine.evaluate(circuit)
    damage_engine = CellularDamageEngine()
    ...
    assert isinstance(n2.epigenetic_marks, dict)
...
def test_mean_gene_count_computed():
    circuit = _make_circuit()
    stress_engine = CellularStressEngine()
    stress_result = stress_engine.evaluate(circuit)
    damage_engine = CellularDamageEngine()
