...
def test_repair_action_model():
    a = RepairAction(cell_id="c1", action="reversible_repair", success=True, energy_cost=0.05, damage_before=0.3, damage_after=0.0)
...
def test_success_rate_computed():
    ...
    repair_engine = CellularRepairEngine()
    result = repair_engine.run(circuit, damage_result.per_cell)
    expected = result.success_count / len(result.actions) if result.actions else 0.0
    assert abs(result.repair_success_rate - expected) < 1e-6
...
def test_no_damage_no_action():
    ...
    repair_engine = CellularRepairEngine()
    result = repair_engine.run(circuit, damage_result.per_cell)
    # "none" damage should still produce an action (no_damage)
    assert len(result.actions) >= 1
...
def test_low_energy_blocks_repair():
    ...
    result = repair_engine.run(circuit, damage_result.per_cell)
    for action in result.actions:
        # Only check actual repair attempts (skip no_damage)
