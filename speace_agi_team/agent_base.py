...
class AgentBase:
    ...
    def chat(self, message: str) -> str:
        ...
        try:
            ...
            # ── OpenAI-compatible endpoint (OpenCode Zen) ─────────────
            if self.config.is_openai_compatible:
                api_path = f"{self.config.endpoint}/chat/completions"
            else:
                api_path = f"{self.config.endpoint}/api/chat"
            ...
            with httpx.Client(timeout=120.0) as client:
                ...
                # ── OpenAI vs Ollama response format ─────────────────
                if self.config.is_openai_compatible:
                    content = (
